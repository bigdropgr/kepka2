...
class ELearning_Ajax {
    ...
    public function submitQuiz(): void {
        ...
        // Parse answers - handle empty answers
        if (empty($answers_json) || $answers_json === '{}') {
            $user_answers = [];
        } else {
            $user_answers = json_decode($answers_json, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                error_log('JSON decode error: ' . json_last_error_msg());
                wp_send_json_error(__('Μη έγκυρα δεδομένα απαντήσεων', 'elearning-quiz'));
                return;
            }
        }
        ...
        // Calculate results with ALL questions
        ...
        // Save detailed answers for ALL questions
        foreach ($questions as $question_index => $question) {
            ...
            if ($user_answer === null) {
                $is_correct = false;
                $user_answer = ''; // Empty answer for database
            } else {
            ...
            error_log("Saving answer for question $question_index - Type: {$question['type']}, Correct: " . ($is_correct ? 'Yes' : 'No') . ", Skipped: " . ($user_answer === '' ? 'Yes' : 'No'));
            
            // CRITICAL: Pass the attempt_id here!
            ELearning_Database::saveQuizAnswer(
                $attempt_id,  // This was missing!
                $question_index,
                $question['type'],
                $question['question'],
                $user_answer,
    ...
    /**
     * Calculate points for matching questions - FIXED VERSION
     */
    ...
    private function formatAnswerForDisplay($answer, $question): string {
        switch ($question['type']) {
            ...
            case 'fill_blanks':
                if (is_array($answer)) {
                    // Return the actual words filled in
            ...
            case 'matching':
                if (is_array($answer)) {
                    ...
                    if (!empty($answer) && isset($answer[0]) && is_array($answer[0]) && isset($answer[0]['left'])) {
                        ...
                        // CRITICAL FIX: Properly iterate through the user's answers
                    ...
                    // Sort matches by left item for consistent display
                    sort($matches);
