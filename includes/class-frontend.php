<?php
/**
 * Frontend Class - VERSION 4.0 WITH ENHANCED FEATURES
 * 
 * Added support for multiple True/False statements
 * Fixed randomization implementation
 * Enhanced quiz rendering with all new features
 */
...
class ELearning_Frontend {
    ...
    public function addQuizContent($content) {
        ...
        if ($associated_lesson) {
            ...
            if (!$lesson_completed) {
                ...
                if ($lesson && $lesson->post_status === 'publish') {
                    ...
                    $content .= '<h2>Κλειδωμένο Κουίζ</h2>';
                    $content .= '<p>Πρέπει να ολοκληρώσετε το <strong>' . esc_html($lesson->post_title) . '</strong> για να δώσετε αυτό το κουίζ.</p>';
        ...
        $content .= '<div class="elearning-quiz-container" data-quiz-id="' . esc_attr($post->ID) . '">';
        $content .= $this->renderQuizInterface($post->ID, $questions, $passing_score, $min_questions, $show_results, $randomize_answers);
        $content .= '</div>';
    ...
    public function addLessonContent($content) {
        ...
        foreach ($sections as $index => $section) {
            ...
            if ($is_accessible) {
                ...
                if (!$is_completed) {
                    ...
                    $content .= 'ΣΗΜΕΙΩΣΗ ΩΣ ΟΛΟΚΛΗΡΩΜΕΝΟ';
                    $content .= '</button>';
                    $content .= '</div>';
                }
        ...
        if ($associated_quiz) {
            ...
            if ($quiz && $quiz->post_status === 'publish') {
                ...
                $content .= '<p>Ολοκληρώσατε όλες τις ενότητες. Δοκιμάστε τις γνώσεις σας με το κουίζ!</p>';
                $content .= '<a href="' . get_permalink($associated_quiz) . '" class="button quiz-button">ΕΝΑΡΞΗ ΚΟΥΙΖ</a>';
                $content .= '</div>';
            }
        }
    ...
    /**
     * Render quiz interface with randomization support
     */
    private function renderQuizInterface($quiz_id, $questions, $passing_score, $min_questions, $show_results, $randomize_answers = 'no') {
        ...
        if ($has_passed) {
            ...
            $html .= '<p>Η καλύτερη βαθμολογία σας: ' . number_format($this->getBestScore($previous_attempts), 1) . '%</p>';
        ...
        // Note: Actual question selection happens on the server side when quiz starts
        // We'll show all questions here, but the server will determine which ones to use
        
        $html .= '<div class="elearning-quiz-intro" ' . ($has_passed ? 'style="display:none;"' : '') . '>';
        $html .= '<div class="quiz-info">';
        $html .= '<div class="quiz-stat"><span class="label">Ερωτήσεις:</span> <span class="value">' . min($min_questions, count($questions)) . '</span></div>';
        ...
        $html .= '</div>';
        $html .= '<button type="button" class="start-quiz-btn" data-quiz-id="' . esc_attr($quiz_id) . '">ΕΝΑΡΞΗ ΚΟΥΙΖ</button>';
        $html .= '</div>';
        
        // Quiz form (hidden initially) - with data attributes for randomization settings
        $html .= '<form class="elearning-quiz-form" style="display: none;" data-passing-score="' . esc_attr($passing_score) . '" data-show-results="' . esc_attr($show_results) . '" data-randomize-answers="' . esc_attr($randomize_answers) . '">';
        ...
        $html .= '<div class="progress-text"><span class="current">1</span> / <span class="total">' . min($min_questions, count($questions)) . '</span></div>';
        $html .= '</div>';
        
        // Questions container - render all questions, JavaScript will handle display order
        $html .= '<div class="quiz-questions-container">';
        
        foreach ($questions as $index => $question) {
            $html .= $this->renderQuestion($index, $question, $randomize_answers);
        }
        ...
        $html .= '<button type="button" class="quiz-nav-btn prev-btn" disabled>ΠΡΟΗΓΟΥΜΕΝΗ</button>';
        $html .= '<button type="button" class="quiz-nav-btn next-btn">ΕΠΟΜΕΝΗ</button>';
        $html .= '<button type="button" class="quiz-submit-btn" style="display: none;">ΥΠΟΒΟΛΗ ΚΟΥΙΖ</button>';
        $html .= '</div>';
    ...
    /**
     * Render individual question with support for multiple T/F
     */
    private function renderQuestion($index, $question, $randomize_answers = 'no') {
        $question_class = $index === 0 ? 'quiz-question active' : 'quiz-question';
        
        $html = '<div class="' . $question_class . '" data-question-index="' . esc_attr($index) . '" data-question-type="' . esc_attr($question['type']) . '" data-randomize-answers="' . esc_attr($randomize_answers) . '">';
        $html .= '<div class="question-header">';
        $html .= '<h3 class="question-title">Ερώτηση ' . ($index + 1) . '</h3>';
        $html .= '</div>';
    ...
    private function renderFillBlanks($index, $question) {
        ...
        // Process text to create blanks while preserving HTML
        $blank_count = 0;
        
        // First, temporarily replace {{blank}} with a unique placeholder to protect it during HTML processing
        $temp_placeholder = '###BLANK_PLACEHOLDER_' . uniqid() . '###';
        $text_with_placeholders = str_replace('{{blank}}', $temp_placeholder, $text_with_blanks);
        
        // Apply WordPress auto-paragraph formatting
        $formatted_text = wpautop($text_with_placeholders);
        
        // Now replace the temporary placeholders with the actual blank spans
        $processed_text = preg_replace_callback('/' . preg_quote($temp_placeholder, '/') . '/', function($matches) use (&$blank_count) {
        ...
        // Use the processed text with HTML preserved
        $html .= '<div class="text-with-blanks">' . $processed_text . '</div>';
    ...
    /**
     * Render true/false question - ENHANCED FOR MULTIPLE STATEMENTS
     */
    private function renderTrueFalse($index, $question) {
        // Check if this is a multiple T/F question
        if (!empty($question['tf_statements']) && is_array($question['tf_statements'])) {
            // Multiple True/False statements
            $html = '<p class="instruction">Για κάθε δήλωση, επιλέξτε Σωστό ή Λάθος:</p>';
            
            $html .= '<div class="tf-statements-container">';
            
            foreach ($question['tf_statements'] as $stmt_index => $statement) {
                $html .= '<div class="tf-statement-item" data-statement-index="' . esc_attr($stmt_index) . '">';
                
                // Statement text
                $html .= '<div class="tf-statement-text">';
                $html .= '<strong>' . ($stmt_index + 1) . '.</strong> ' . esc_html($statement['text']);
                $html .= '</div>';
                
                // True/False options for this statement
                $html .= '<div class="tf-statement-options">';
                
                $html .= '<label class="option-label neutral-option">';
                $html .= '<input type="radio" name="questions[' . $index . '][tf_answers][' . $stmt_index . ']" value="true" />';
                $html .= '<span class="option-text">Σωστό</span>';
                $html .= '</label>';
                
                $html .= '<label class="option-label neutral-option">';
                $html .= '<input type="radio" name="questions[' . $index . '][tf_answers][' . $stmt_index . ']" value="false" />';
            ...
            // Add CSS for multiple T/F statements
            $html .= '<style>
            ...
            // Single True/False question (original implementation)
    ...
    private function renderMatching($index, $question) {
        ...
        $html = '<p class="instruction">Σύρετε τα στοιχεία από τη δεξιά στήλη για να τα αντιστοιχίσετε με τα στοιχεία στην αριστερή στήλη:</p>';
        ...
        <div id="quiz-confirmation-modal" class="quiz-modal" style="display: none;">
            <div class="modal-content">
                ...
                <div class="modal-buttons">
                    <button type="button" class="button secondary" id="cancel-submit">ΑΚΥΡΩΣΗ</button>
                    <button type="button" class="button primary" id="confirm-submit">ΥΠΟΒΟΛΗ ΚΟΥΙΖ</button>
                </div>
            </div>
        </div>
        <?php
    ...
    private function getLessonTrackingScript() {
        ...
        jQuery(document).ready(function($) {
            ...
            $(document).on('click', '.mark-complete-btn', function(e) {
                ...
                $button.prop('disabled', true).text('Σημείωση ως Ολοκληρωμένο...');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            alert(response.data || 'Σφάλμα ενημέρωσης προόδου');
                            $button.prop('disabled', false).text('ΣΗΜΕΙΩΣΗ ΩΣ ΟΛΟΚΛΗΡΩΜΕΝΟ');
                        }
                    },
                    error: function() {
                        alert('Σφάλμα ενημέρωσης προόδου');
                        $button.prop('disabled', false).text('ΣΗΜΕΙΩΣΗ ΩΣ ΟΛΟΚΛΗΡΩΜΕΝΟ');
                    }
                });
            });
