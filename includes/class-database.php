 ...
 * Updated with Greek language support
 */
...
class ELearning_Database {
    ...
    public static function getQuizStatistics($quiz_id): array {
        ...
        $stats = $wpdb->get_row($wpdb->prepare(
            "SELECT 
                ...
                COUNT(CASE WHEN language = 'el' OR language = 'gr' THEN 1 END) as greek_attempts,
    ...
    public static function getGlobalStatistics(): array {
        ...
        $stats = $wpdb->get_row(
            "SELECT 
                ...
                COUNT(CASE WHEN language = 'el' OR language = 'gr' THEN 1 END) as greek_total
             FROM $attempts_table",
            ARRAY_A
        );
    ...
    /**
     * Get current language for Greek support and WPML compatibility
     */
    private static function getCurrentLanguage(): string {
        // Check for WPML
        if (function_exists('icl_get_current_language')) {
            $lang = icl_get_current_language();
            // Convert WPML Greek code to our format
            if ($lang === 'el') {
                return 'el';
            }
            return $lang;
        ...
        // Check plugin settings for default language
        $settings = get_option('elearning_quiz_settings', []);
        if (isset($settings['default_language']) && $settings['default_language'] === 'el') {
            return 'el';
        }
    ...
    /**
     * Export quiz data to CSV with improved format and Greek support
     */
    public static function exportQuizData($quiz_id, $start_date = null, $end_date = null): string {
        ...
        // Create CSV content with BOM for Greek characters
        $csv_content = "\xEF\xBB\xBF"; // UTF-8 BOM
        
        // Add headers (bilingual)
        $headers = [
            __('Attempt ID', 'elearning-quiz') . ' / ID Προσπάθειας',
            __('User Session', 'elearning-quiz') . ' / Συνεδρία Χρήστη',
            __('Language', 'elearning-quiz') . ' / Γλώσσα',
            __('Start Time', 'elearning-quiz') . ' / Ώρα Έναρξης',
            __('End Time', 'elearning-quiz') . ' / Ώρα Λήξης',
            __('Time Spent (seconds)', 'elearning-quiz') . ' / Χρόνος (δευτερόλεπτα)',
            __('Status', 'elearning-quiz') . ' / Κατάσταση',
            __('Score (%)', 'elearning-quiz') . ' / Βαθμολογία (%)',
            __('Total Questions', 'elearning-quiz') . ' / Σύνολο Ερωτήσεων',
            __('Correct Answers', 'elearning-quiz') . ' / Σωστές Απαντήσεις',
            __('Passed', 'elearning-quiz') . ' / Επιτυχία',
            __('Duration (minutes)', 'elearning-quiz') . ' / Διάρκεια (λεπτά)'
        ];
        $csv_content .= implode(',', $headers) . "\n";
        
        // Add data rows
        foreach ($results as $row) {
            // Translate status and passed values
            $row['status'] = $row['status'] === 'completed' ? __('Completed', 'elearning-quiz') . ' / Ολοκληρώθηκε' : 
                           ($row['status'] === 'started' ? __('Started', 'elearning-quiz') . ' / Ξεκίνησε' : 
                           __('Abandoned', 'elearning-quiz') . ' / Εγκαταλείφθηκε');
            $row['passed'] = $row['passed'] ? __('Yes', 'elearning-quiz') . ' / Ναι' : __('No', 'elearning-quiz') . ' / Όχι';
            $row['language'] = $row['language'] === 'el' ? 'Ελληνικά' : 'English';
