<?php
/**
 * Custom Post Types Class - ENHANCED VERSION 4.0
 * 
 * Handles registration of custom post types and taxonomies
 * ENHANCED: Multiple True/False statements support
 * Greek only version
 */
...
class ELearning_Post_Types {
    ...
    private function renderSectionFields($index, $section): void {
        ...
        <div class="lesson-section" data-index="<?php echo esc_attr($index); ?>">
            <div class="section-header">
                <h4><?php echo is_numeric($index) ? 'Ενότητα ' . ($index + 1) : 'Ενότητα 1'; ?></h4>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if (is_numeric($index)) {
                            ...
                            <p class="description">Ο επεξεργαστής εμπλουτισμένου κειμένου θα αρχικοποιηθεί όταν προστεθεί η ενότητα.</p>
                            <?php
                        }
                        ?>
                    </td>
    ...
    public function renderLessonQuizMetaBox($post): void {
        ...
        echo '</select>';
        echo '<p class="description">Επιλέξτε ένα κουίζ για να συσχετιστεί με αυτό το μάθημα. Οι μαθητές θα κάνουν αυτό το κουίζ μετά την ολοκλήρωση του μαθήματος.</p>';
        echo '</td>';
        echo '</tr>';
        echo '</table>';
    }
    ...
    public function renderQuizSettingsMetaBox($post): void {
        ...
        <table class="form-table">
            <tr>
                <th><label for="passing_score">Βαθμός Επιτυχίας (%)</label></th>
                <td>
                    <input type="number" 
                           id="passing_score" 
                           name="passing_score" 
            ...
            <tr>
                <th><label for="min_questions_to_show">Ερωτήσεις προς Εμφάνιση</label></th>
                <td>
                    <input type="number" 
                           ...
                           min="1" 
                           step="1" />
                    <p class="description">Αριθμός ερωτήσεων που θα εμφανίζονται από την τράπεζα ερωτήσεων. Αφήστε κενό για εμφάνιση όλων.</p>
                </td>
            </tr>
            <tr>
                <th><label for="time_limit">Χρονικό Όριο (λεπτά)</label></th>
                <td>
                    <input type="number" 
                           id="time_limit" 
                           name="time_limit" 
            ...
            <tr>
                <th><label for="show_results_immediately">Εμφάνιση Αποτελεσμάτων</label></th>
                <td>
                    <select name="show_results_immediately" id="show_results_immediately">
                        <option value="yes" <?php selected($show_results, 'yes'); ?>>Ναι</option>
                        <option value="no" <?php selected($show_results, 'no'); ?>>Όχι</option>
                    </select>
                    <p class="description">
                        Ναι: Εμφάνιση σωστών απαντήσεων μετά την υποβολή<br>
                        Όχι: Εμφάνιση μόνο του σκορ
                    </p>
                </td>
            </tr>
            <tr>
                <th><label for="randomize_questions">Τυχαία Σειρά Ερωτήσεων</label></th>
                <td>
                    <select name="randomize_questions" id="randomize_questions">
                        <option value="yes" <?php selected($randomize_questions, 'yes'); ?>>Ναι</option>
                        <option value="no" <?php selected($randomize_questions, 'no'); ?>>Όχι</option>
                    </select>
                    <p class="description">Εμφάνιση ερωτήσεων σε τυχαία σειρά για κάθε προσπάθεια.</p>
                </td>
            </tr>
            <tr>
                <th><label for="randomize_answers">Τυχαία Σειρά Απαντήσεων</label></th>
                <td>
                    <select name="randomize_answers" id="randomize_answers">
                        <option value="yes" <?php selected($randomize_answers, 'yes'); ?>>Ναι</option>
                        <option value="no" <?php selected($randomize_answers, 'no'); ?>>Όχι</option>
                    </select>
    ...
    private function renderQuestionFields($index, $question): void {
        ...
        $question = wp_parse_args($question, [
            ...
            'options' => [''],
            'correct_answers' => [],
            'tf_statements' => [] // For multiple T/F
        ]);
        
        ?>
        <div class="quiz-question" data-index="<?php echo esc_attr($index); ?>">
            <div class="question-header">
                <h4><?php echo is_numeric($index) ? 'Ερώτηση ' . ($index + 1) : 'Ερώτηση 1'; ?></h4>
    ...
    private function renderFillBlanksOptions($index, $question): void {
        ...
        $editor_id = 'fill_blanks_text_' . $index;
        wp_editor($text_with_blanks, $editor_id, [
            'textarea_name' => 'quiz_questions[' . esc_attr($index) . '][text_with_blanks]',
            'textarea_rows' => 8,
            'media_buttons' => true,
            'teeny' => false,
            'quicktags' => true,
            'tinymce' => [
                'toolbar1' => 'formatselect,bold,italic,underline,separator,alignleft,aligncenter,alignright,separator,link,unlink,undo,redo',
                'toolbar2' => 'bullist,numlist,separator,outdent,indent,separator,forecolor,backcolor,separator,code',
            ]
        ]);
        ?>
        
        <h5 style="margin-top: 20px;">Τράπεζα Λέξεων</h5>
    ...
    /**
     * Render true/false options - ENHANCED FOR MULTIPLE STATEMENTS
     */
    private function renderTrueFalseOptions($index, $question): void {
        ?>
        <div class="tf-options-wrapper">
            <h5>Επιλέξτε τύπο Σωστό/Λάθος</h5>
            
            <label>
                <input type="radio" 
                       name="quiz_questions[<?php echo esc_attr($index); ?>][tf_type]" 
                       value="single" 
                       <?php checked(empty($question['tf_statements']) || !is_array($question['tf_statements'])); ?> 
                       class="tf-type-selector" />
                Μονή Ερώτηση Σωστό/Λάθος
            </label>
            <br><br>
            ...
            <label>
                <input type="radio" 
                       ...
                       <?php checked(!empty($question['tf_statements']) && is_array($question['tf_statements'])); ?> 
                       class="tf-type-selector" />
                Πολλαπλές Δηλώσεις Σωστό/Λάθος
            </label>
            
            <!-- Single T/F -->
            <div class="tf-single-container" style="<?php echo (empty($question['tf_statements']) || !is_array($question['tf_statements'])) ? '' : 'display:none;'; ?>">
            ...
            <div class="tf-multiple-container" style="<?php echo (!empty($question['tf_statements']) && is_array($question['tf_statements'])) ? '' : 'display:none;'; ?>">
                <h5>Δηλώσεις Σωστό/Λάθος</h5>
                <p class="description">Προσθέστε πολλαπλές δηλώσεις. Κάθε δήλωση θα αξιολογηθεί ως Σωστό ή Λάθος.</p>
                
                <div class="tf-statements-container">
                    <?php
                    $tf_statements = isset($question['tf_statements']) && is_array($question['tf_statements']) ? $question['tf_statements'] : [];
                    if (empty($tf_statements)) {
                        $tf_statements = [['text' => '', 'answer' => 'true']];
                    }
                    
                    foreach ($tf_statements as $stmt_index => $statement):
                        $stmt_text = isset($statement['text']) ? $statement['text'] : '';
                        $stmt_answer = isset($statement['answer']) ? $statement['answer'] : 'true';
                    ?>
                        <div class="tf-statement">
                            <input type="text" 
                                   name="quiz_questions[<?php echo esc_attr($index); ?>][tf_statements][<?php echo esc_attr($stmt_index); ?>][text]" 
                                   value="<?php echo esc_attr($stmt_text); ?>" 
                                   placeholder="Δήλωση" 
                                   class="regular-text" />
                            
                            <label class="tf-radio-label">
                                <input type="radio" 
                                       name="quiz_questions[<?php echo esc_attr($index); ?>][tf_statements][<?php echo esc_attr($stmt_index); ?>][answer]" 
                                       value="true" 
                                       <?php checked($stmt_answer, 'true'); ?> />
                                Σωστό
                            </label>
                            ...
                            <label class="tf-radio-label">
                                <input type="radio" 
                                       name="quiz_questions[<?php echo esc_attr($index); ?>][tf_statements][<?php echo esc_attr($stmt_index); ?>][answer]" 
                                       value="false" 
                                       <?php checked($stmt_answer, 'false'); ?> />
                                Λάθος
                            </label>
                            
                            <button type="button" class="remove-tf-statement button-link-delete">Αφαίρεση</button>
                        </div>
                    <?php endforeach; ?>
                </div>
                
                <button type="button" class="add-tf-statement button">Προσθήκη Δήλωσης</button>
            </div>
        </div>
        
        <script>
        jQuery(document).ready(function($) {
            // Handle T/F type switching
            $(document).on('change', '.tf-type-selector', function() {
                const $wrapper = $(this).closest('.tf-options-wrapper');
                const type = $(this).val();
                
                if (type === 'single') {
                    $wrapper.find('.tf-single-container').show();
                    $wrapper.find('.tf-multiple-container').hide();
                } else {
                    $wrapper.find('.tf-single-container').hide();
                    $wrapper.find('.tf-multiple-container').show();
                }
            });
        });
        </script>
    ...
    /**
     * Save quiz meta data - ENHANCED VERSION
     */
    private function saveQuizMeta($post_id): void {
        ...
        // Save quiz questions with validation - ENHANCED VERSION
            ...
            if (isset($_POST['quiz_questions']) && is_array($_POST['quiz_questions'])) {
                ...
                foreach ($_POST['quiz_questions'] as $question_data) {
                    ...
                    switch ($question['type']) {
                        ...
                        case 'fill_blanks':
                            // UPDATED: Use wp_kses_post instead of sanitize_textarea_field to preserve HTML
                            $text_with_blanks = wp_kses_post($question_data['text_with_blanks'] ?? '');
                            $word_bank = [];
                        ...
                        case 'true_false':
                            // ENHANCED: Handle multiple T/F statements
                            $tf_type = isset($question_data['tf_type']) ? $question_data['tf_type'] : 'single';
                            
                            if ($tf_type === 'multiple' && isset($question_data['tf_statements']) && is_array($question_data['tf_statements'])) {
                                // Multiple T/F statements
                                $tf_statements = [];
                                
                                foreach ($question_data['tf_statements'] as $statement) {
                                    if (!empty($statement['text'])) {
                                        $tf_statements[] = [
                                            'text' => sanitize_text_field($statement['text']),
                                            'answer' => in_array($statement['answer'], ['true', 'false']) ? $statement['answer'] : 'true'
                                        ];
                                    }
                                }
                                
                                if (!empty($tf_statements)) {
                                    $question['tf_statements'] = $tf_statements;
                                } else {
                                    continue 2; // Skip invalid question
                                }
                            } else {
            ...
            case 'associated_quiz':
                ...
                if ($quiz_id) {
                    ...
                    if ($quiz && $quiz->post_status !== 'trash') {
                        $status_text = $quiz->post_status === 'publish' ? '' : ' (' . $this->getPostStatusInGreek($quiz->post_status) . ')';
            ...
            case 'associated_lesson':
                ...
                if ($lesson_id) {
                    ...
                    if ($lesson && $lesson->post_status !== 'trash') {
                        $status_text = $lesson->post_status === 'publish' ? '' : ' (' . $this->getPostStatusInGreek($lesson->post_status) . ')';
    ...
    /**
     * Get post status in Greek
     */
    private function getPostStatusInGreek($status): string {
        $statuses = [
            'draft' => 'Πρόχειρο',
            'pending' => 'Σε Αναμονή',
            'private' => 'Ιδιωτικό',
