...
class ELearning_Import_Export {
    ...
    public function handleQuestionsImport(): void {
        ...
        if (!wp_verify_nonce($_POST['nonce'] ?? '', 'elearning_import_nonce')) {
            wp_send_json_error('Η επαλήθευση ασφαλείας απέτυχε');
        }
        
        if (!current_user_can('edit_elearning_quizzes')) {
            wp_send_json_error('Δεν έχετε άδεια να εισάγετε ερωτήσεις');
        }
        ...
        if (empty($_FILES['import_file'])) {
            wp_send_json_error('Δεν ανέβηκε κανένα αρχείο');
        }
        ...
        if (!in_array($file['type'], $allowed_types)) {
            wp_send_json_error('Παρακαλώ ανεβάστε ένα αρχείο CSV');
        }
        ...
        if (empty($questions)) {
            wp_send_json_error('Δεν βρέθηκαν έγκυρες ερωτήσεις στο αρχείο');
        }
        ...
        wp_send_json_success([
            'message' => 'Εισήχθησαν επιτυχώς ' . count($questions) . ' ερωτήσεις',
    ...
    public function handleQuestionsExport(): void {
        ...
        if (!wp_verify_nonce($_GET['nonce'] ?? '', 'elearning_export_nonce')) {
            wp_die('Η επαλήθευση ασφαλείας απέτυχε');
        }
        
        if (!current_user_can('export_elearning_data')) {
            wp_die('Δεν έχετε άδεια να εξάγετε ερωτήσεις');
        }
        ...
        if (empty($questions)) {
            wp_die('Δεν βρέθηκαν ερωτήσεις σε αυτό το κουίζ');
        }
    ...
    private function parseQuestionRow($data, $header_map): ?array {
        // Required fields - check both English and Greek headers
        $type_index = $header_map['type'] ?? $header_map['τύπος'] ?? $header_map['question type'] ?? null;
        $question_index = $header_map['question'] ?? $header_map['ερώτηση'] ?? $header_map['question text'] ?? null;
        ...
        // Map CSV type to internal type (support both English and Greek)
        $type_mapping = [
            'multiple choice' => 'multiple_choice',
            'multiple_choice' => 'multiple_choice',
            'πολλαπλής επιλογής' => 'multiple_choice',
    ...
    private function parseMultipleChoiceData($data, $header_map, $question): array {
        ...
        // Look for option columns (Option 1, Option 2, etc.) - support Greek headers
        for ($i = 1; $i <= 10; $i++) {
            $option_keys = [
                'option ' . $i,
                'option' . $i,
        ...
        // Get correct answers
        $correct_keys = ['correct', 'correct answers', 'correct answer', 'σωστή', 'σωστές απαντήσεις', 'σωστή απάντηση'];
        $correct_index = null;
        foreach ($correct_keys as $key) {
            if (isset($header_map[$key])) {
                $correct_index = $header_map[$key];
                break;
            }
        }
    ...
    private function parseTrueFalseData($data, $header_map, $question): array {
        $correct_keys = ['correct', 'correct answer', 'answer', 'σωστή', 'σωστή απάντηση', 'απάντηση'];
        $correct_index = null;
        ...
        if ($correct_index !== null && !empty($data[$correct_index])) {
            ...
            // Support both English and Greek answers
            if (in_array($answer, ['true', 't', '1', 'yes', 'σωστό', 'σ', 'ναι'])) {
                $question['correct_answer'] = 'true';
            } elseif (in_array($answer, ['false', 'f', '0', 'no', 'λάθος', 'λ', 'όχι'])) {
    ...
    private function parseFillBlanksData($data, $header_map, $question): array {
        ...
        $text_keys = ['text with blanks', 'blank text', 'κείμενο με κενά', 'κείμενο κενών'];
        $text_index = null;
        
        foreach ($text_keys as $key) {
            if (isset($header_map[$key])) {
                $text_index = $header_map[$key];
                break;
            }
        }
        ...
        // Get word bank
        $word_bank = [];
        $bank_keys = ['word bank', 'words', 'τράπεζα λέξεων', 'λέξεις'];
        $bank_index = null;
        
        foreach ($bank_keys as $key) {
            if (isset($header_map[$key])) {
                $bank_index = $header_map[$key];
                break;
            }
        }
        ...
        if ($bank_index !== null && !empty($data[$bank_index])) {
            ...
            for ($i = 1; $i <= 10; $i++) {
                $word_keys = ['word ' . $i, 'word' . $i, 'λέξη ' . $i, 'λέξη' . $i];
                $word_index = null;
                
                foreach ($word_keys as $key) {
                    if (isset($header_map[$key])) {
                        $word_index = $header_map[$key];
                        break;
                    }
                }
    ...
    private function parseMatchingData($data, $header_map, $question): array {
        ...
        for ($i = 1; $i <= 10; $i++) {
            $left_keys = ['left ' . $i, 'left' . $i, 'αριστερά ' . $i, 'αριστερά' . $i];
            $left_index = null;
            
            foreach ($left_keys as $key) {
                if (isset($header_map[$key])) {
                    $left_index = $header_map[$key];
                    break;
                }
            }
        ...
        for ($i = 1; $i <= 10; $i++) {
            $right_keys = ['right ' . $i, 'right' . $i, 'δεξιά ' . $i, 'δεξιά' . $i];
            $right_index = null;
            
            foreach ($right_keys as $key) {
                if (isset($header_map[$key])) {
                    $right_index = $header_map[$key];
                    break;
                }
            }
        ...
        // Get matches
        $matches_keys = ['matches', 'correct matches', 'αντιστοιχίσεις', 'σωστές αντιστοιχίσεις'];
        $matches_index = null;
        
        foreach ($matches_keys as $key) {
            if (isset($header_map[$key])) {
                $matches_index = $header_map[$key];
                break;
            }
        }
    ...
    private function generateQuestionsCSV($questions): string {
        ...
        $headers = [
            ...
            'Επιλογή 4 / Option 4',
            'Επιλογή 5 / Option 5',
            'Σωστή(ές) Απάντηση(εις) / Correct Answer(s)',
            'Κείμενο με Κενά / Text with Blanks',
            'Τράπεζα Λέξεων / Word Bank',
            'Αριστερά 1 / Left 1',
            'Αριστερά 2 / Left 2',
            'Αριστερά 3 / Left 3',
            'Αριστερά 4 / Left 4',
    ...
    private function questionToCSVRow($question): array {
        ...
        $row[0] = $type_names[$question['type']] ?? $question['type'];
        ...
        switch ($question['type']) {
            ...
            case 'true_false':
                ...
                $row[7] = $answer_map[$question['correct_answer']] ?? $question['correct_answer'];
                break;
    ...
    public function addImportModal(): void {
            ...
            <div class="modal-content">
                ...
                <form id="import-questions-form" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="import-file">Επιλέξτε Αρχείο CSV:</label>
                    ...
                    <div class="form-group">
                        ...
                        <label>
                            ...
                            Προσθήκη στις υπάρχουσες ερωτήσεις
                        </label>
                        <label>
                            <input type="radio" name="import_mode" value="replace">
                            Αντικατάσταση υπαρχουσών ερωτήσεων
                        </label>
                    </div>
        ...
        jQuery(document).ready(function($) {
            $('#import-questions-form').on('submit', function(e) {
                ...
                $submitBtn.prop('disabled', true).text('Εισαγωγή...');
                ...
                $.ajax({
                    ...
                    error: function() {
                        $('#import-result').html('<div class="notice notice-error"><p>Η εισαγωγή απέτυχε. Παρακαλώ δοκιμάστε ξανά.</p></div>').show();
                        $submitBtn.prop('disabled', false).text(originalText);
                    }
                });
            });
