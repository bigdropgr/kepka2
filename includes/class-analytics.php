...
class ELearning_Analytics {
    ...
    public function addDashboardWidget(): void {
        if (current_user_can('view_elearning_analytics')) {
            wp_add_dashboard_widget(
                ...
                __('Επισκόπηση Συστήματος E-Learning', 'elearning-quiz'),
    ...
    public function renderDashboardWidget(): void {
        ...
        <div class="elearning-dashboard-widget">
            <div class="widget-stats">
                <div class="stat-item">
                    <span class="stat-label"><?php _e('Συνολικές Προσπάθειες', 'elearning-quiz'); ?></span>
                    <span class="stat-value"><?php echo number_format($stats['total_attempts']); ?></span>
                </div>
                <div class="stat-item">
                    <span class="stat-label"><?php _e('Ποσοστό Επιτυχίας', 'elearning-quiz'); ?></span>
                    <span class="stat-value"><?php echo number_format($stats['pass_rate'], 1); ?>%</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label"><?php _e('Μέση Βαθμολογία', 'elearning-quiz'); ?></span>
            ...
            <div class="widget-actions">
                <a href="<?php echo admin_url('admin.php?page=elearning-analytics'); ?>" class="button button-small">
                    <?php _e('Προβολή Πλήρων Αναλυτικών', 'elearning-quiz'); ?>
                </a>
            </div>
        </div>
    ...
    public function getQuizPerformance(): void {
        ...
        if (!current_user_can('view_elearning_analytics')) {
            wp_send_json_error(__('Δεν επιτρέπεται η πρόσβαση', 'elearning-quiz'));
        }
    ...
    private function getQuizPerformanceData($quiz_id): array {
        ...
        $basic_stats = $wpdb->get_row($wpdb->prepare(
            "SELECT 
                ...
                COUNT(CASE WHEN language = 'gr' OR language = 'el' THEN 1 END) as greek_attempts
             FROM $attempts_table 
             WHERE quiz_id = %d",
            $quiz_id
        ), ARRAY_A);
    ...
    public function getChartData(): void {
        ...
        switch ($chart_type) {
            ...
            case 'question_difficulty':
                ...
                break;
            default:
                wp_send_json_error(__('Μη έγκυρος τύπος γραφήματος', 'elearning-quiz'));
        }
    ...
    private function getLanguageBreakdownData($quiz_id = 0): array {
        ...
        $results = $wpdb->get_results(
            "SELECT 
                CASE 
                    WHEN language = 'en' THEN 'Αγγλικά'
                    WHEN language = 'gr' OR language = 'el' THEN 'Ελληνικά'
                    ELSE 'Άλλη'
    ...
    public function getMostPopularQuizzes($limit = 10): array {
        ...
        foreach ($results as &$result) {
            ...
            $result['quiz_title'] = $quiz ? $quiz->post_title : __('Άγνωστο Κουίζ', 'elearning-quiz');
